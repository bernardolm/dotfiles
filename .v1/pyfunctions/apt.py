...
from pyfunctions.zsh import log_debug, log_error, log_finish, log_info, log_start
...
def apt_recovery_pub_keys(argv: list[str]) -> int:
	...
	curl_result = run(["curl", "-fsSL", "https://downloads.1password.com/linux/keys/1password.asc"], capture=True)
	run(["sudo", "gpg", "--yes", "--dearmor", "--output", str(file_path)], input_data=curl_result.stdout)
	run(["sudo", "/bin/rm", "-f", "/etc/apt/sources.list.d/1password.list"])
	run(
		["sudo", "tee", "/etc/apt/sources.list.d/1password.list"],
		input_data="deb [arch=amd64 signed-by=/etc/apt/trusted.gpg.d/1password.gpg] https://downloads.1password.com/linux/debian/amd64 stable main",
	)
	run(["sudo", "mkdir", "-p", "/etc/debsig/policies/AC2D62742012EA22"])
	pol = run(["curl", "-fsSL", "https://downloads.1password.com/linux/debian/debsig/1password.pol"], capture=True).stdout
	run(["sudo", "tee", "/etc/debsig/policies/AC2D62742012EA22/1password.pol"], input_data=pol)
	...
	curl_result = run(["curl", "-fsSL", "https://packages.cloud.google.com/apt/doc/apt-key.gpg"], capture=True)
	run(["sudo", "gpg", "--yes", "--dearmor", "--output", str(file_path)], input_data=curl_result.stdout)
	...
	for server in servers:
		...
		result = run(["sudo", "apt-key", "adv", "--keyserver", server, "--recv-keys", key], capture=True)
		if result.returncode == 0:
			break
		log_error([f"failed to {key} from {server}"])

	export_result = run(["sudo", "apt-key", "export", key], capture=True)
	run(["sudo", "gpg", "--batch", "--yes", "--dearmour", "-o", f"/etc/apt/trusted.gpg.d/{key}.gpg"], input_data=export_result.stdout)
	log_info([f"{key} OK"])
